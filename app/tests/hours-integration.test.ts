...
function simulateFinalization(submissions: Array<{ hoursPerDay: number; hoursPerWeek: number | null; score: number; meu: number; projectName: string; projectType: 'mandate' | 'dde' | 'pitch' }>, capacityMeu: number) {
  ...
  const totalHpw = submissions.reduce((sum, s) => sum + (s.hoursPerWeek ?? s.hoursPerDay * 5), 0);
...
describe('Submission pipeline: hours conversion', () => {
  it('converts per_day to per_week correctly (4 hrs/day → 20 hrs/week)', () => {
  ...
  it('passes per_week through unchanged (15 hrs/week → 15 hrs/week)', () => {
    ...
    expect(result.hoursPerDay).toBe(3); // 15/5
  ...
  it('handles fractional hours (1.5 hrs/day → 7.5 hrs/week)', () => {
    const result = simulateSubmission(1.5, 'per_day', 'pitch');
    expect(result.hoursPerDay).toBe(1.5);
    expect(result.hoursPerWeek).toBe(7.5);
  });
...
describe('Conflict resolution: hoursPerWeek update', () => {
  it('resolvedHours * 5 matches normalizeToHoursPerWeek for per_day', () => {
    // Resolve route uses resolvedHours * 5
    // Submit route uses normalizeToHoursPerWeek(value, 'per_day')
    // These must be equivalent
    const resolvedHours = 3.5;
    const fromResolve = resolvedHours * 5;
  ...
  it('re-scoring with resolved hours produces consistent MEU + hours', () => {
    ...
    const hoursPerWeek = resolvedHours * 5;
    expect(score).toBe(3);
    expect(meu).toBe(1.00);
    expect(hoursPerWeek).toBe(25);
  });
});
...
describe('Finalization: snapshot creation with both methods', () => {
  ...
  it('handles null hoursPerWeek with fallback to hoursPerDay * 5', () => {
    ...
    // Fallback: 4*5 + 1*5 = 25
    expect(result.totalHpw).toBe(25);
    expect(result.hoursUtilPct).toBeCloseTo(25 / 84, 4);
  });
});
...
describe('ProjectBreakdownItem: hoursPerWeek field', () => {
  ...
  it('computes hoursPerWeek from fallback for old submissions', () => {
    ...
    const result = simulateFinalization(submissions, 3.0);
    expect(result.breakdown[0].hoursPerWeek).toBe(30); // 6 * 5
  });
});
...
describe('Edge cases: extreme values', () => {
  ...
  it('very small hours (0.1 hrs/day) produce correct per_week', () => {
    ...
    expect(result.hoursPerWeek).toBeCloseTo(0.5);
  });
  ...
  it('large per_week value (40 hrs/week) passes through correctly', () => {
    ...
    expect(result.score).toBe(5); // ≥8 hrs/day
    expect(result.meu).toBe(1.50);
  });
});
...
describe('Consistency: per_day and per_week inputs produce same hours/week', () => {
  it('20 hrs/week submitted as per_week matches 4 hrs/day submitted as per_day', () => {
    const fromPerWeek = simulateSubmission(20, 'per_week', 'mandate');
  ...
  it('7.5 hrs/week submitted as per_week matches 1.5 hrs/day submitted as per_day', () => {
    const fromPerWeek = simulateSubmission(7.5, 'per_week', 'dde');
